...
const step2Schema = z.object({
  code: z.string().length(4, 'Код має складатися з 4 цифр').regex(/^\d+$/, 'Тільки цифри'),
})
...
export default function ResetPasswordPage() {
  ...
  async function onStep1(values: Step1) {
    setServerError(null)
    try {
      await authApi.resetPasswordInitialPhone(values.phone)
      setPhone(values.phone)
      setStep(2)
    } catch (e) {
      setServerError(getErrMsg(e) || 'Помилка. Перевірте номер і спробуйте ще раз.')
    }
  }
  ...
  async function onStep2(values: Step2) {
    setServerError(null)
    try {
      await authApi.resetPasswordConfirmPhone(phone, values.code)
      setOtp(values.code)
      setStep(3)
    } catch (e) {
      setServerError(getErrMsg(e) || 'Невірний код. Спробуйте ще раз.')
    }
  }
  ...
  async function onStep3(values: Step3) {
    setServerError(null)
    try {
      await authApi.resetPasswordFinalPhone(phone, otp, values.password)
      router.push('/auth/login?reset=success')
    } catch (e) {
      setServerError(getErrMsg(e) || 'Помилка. Спробуйте ще раз.')
    }
  }

  const STEPS = ['Телефон', 'Підтвердження', 'Новий пароль']
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="mt-4 text-2xl font-semibold text-foreground">Відновлення пароля</h1>
        </div>
            ...
            <div className="space-y-1.5">
              ...
              {form3.formState.errors.passwordConfirm && (
                <p className="text-xs text-destructive">
                  {form3.formState.errors.passwordConfirm.message}
                </p>
              )}
            </div>
            {serverError && <p className="text-sm text-destructive text-center">{serverError}</p>}
            <Button type="submit" disabled={form3.formState.isSubmitting} className="w-full">
              {form3.formState.isSubmitting ? 'Збереження…' : 'Зберегти пароль'}
            </Button>
          </form>
        )}
